...
>
> *Примечание для пользователей:* Режим **Monitor Mode** может полностью скрывать веб-интерфейс ноды как на **ArcaneOS**, так и **Legacy FluxOS**. Благодаря этому вы можете сфокусироваться на мониторинге событий через полноэкранный лог. Основной функционал - определение входа в систему и автоматическая остановка приложений - работает корректно.

**Профессиональный инструмент мониторинга и управления для узлов Flux.**
...
![Скриншот Flux Node Manager v1.3.0](assets/screenshot.png)

## Скачивание и Установка

Последняя версия: **v1.3.0**

**[Скачать из GitHub Releases](https://github.com/SfilD/flux-node-manager/releases/latest)**

Вы можете выбрать:
*   **Portable (.zip):** Установка не требуется. Просто распакуйте и запустите `FluxNodeManager_Portable_x.y.z.exe`.
*   **Portable (.exe):** Одиночный исполняемый файл для быстрого обновления текущей версии без распаковки архива.
...
1.  **Main Process (`monitor-main.js`)**
    ...
    *   Запускает основной цикл автоматизации для управления приложениями.
    *   Шифрует и хранит чувствительные токены в памяти, используя `safeStorage`.
    *   Рассылает обновления статуса авторизации в реальном времени.

2.  **Renderer Process (UI)**
    *   **Shell (`shell.html`, `shell-renderer.js`):** Главное окно приложения. Предоставляет навигацию по вкладкам с **цветовой индикацией статуса** (Зеленый = OK), панель быстрого доступа и двурежимный просмотр логов (**Login/Monitor Mode**). Общается с основным процессом через защищенный `contextBridge` (`window.electronAPI`).
    *   **Preloader (`preloader.html`, `preloader-renderer.js`):** Простое окно, отображаемое во время начального поиска узлов.

3.  **Preload Scripts**
    *   **`monitor-preload.js`:** Внедряется в `BrowserView` каждого узла Flux. Его основная обязанность - опрашивать `localStorage` для обнаружения токена `zelidauth` когда пользователь входит в систему. Затем он отправляет этот токен в основной процесс для запуска цикла управления. Также внедряет CSS для скрытия лишних элементов интерфейса с веб-страницы узла.
    *   **`shell-preload.js` & `preloader-preload.js`:** Эти скриты безопасно открывают необходимые каналы IPC (`send`, `on`) для своих процессов рендеринга, используя `contextBridge`, в соответствии с современными стандартами безопасности Electron (`contextIsolation: true`).
...
-   `GET /apps/listrunningapps`
    *   **Цель:** Используется как для проверки работоспособности узла на этапе поиска, так и для получения списка запущенных приложений для проверки политик.
    *   **Авторизация:** Не требуется.

-   `GET /apps/appremove?appname={appName}`
    *   **Цель:** Отправляет команду на остановку/очистку определенного контейнера приложения, если он нарушает политику.
    *   **Авторизация:** **Требуется.** Этот запрос использует заголовок `zelidauth` с токеном, перехваченным `monitor-preload.js`, чтобы гарантировать, что действие выполняется владельцем узла.
...
-   **Проблема: Цикл управления не запускается для конкретного узла.**
    *   **Решение:** Убедитесь, что вы успешно вошли в веб-интерфейс узла Flux на соответствующей вкладке в приложении. Цикл управления начинается только после обнаружения валидного токена `zelidauth`.
...
-   **Проблема: Нужно сбросить все данные/кэш приложения.**
    *   **Решение:** Запустите скрипт `clean-session.bat` (входит в комплект). Это полностью удалит директорию данных приложения (`%AppData%\flux-node-manager`), эффективно сбросив его к начальному состоянию.
...
- **Рабочая директория на Windows:** На виртуальной машине Windows проект расположен по пути `C:\Projects\flux-node-manager\`.
...
Для обновления версии приложения выполните следующую команду в **среде WSL**:
...
1.  Обновит версию в `package.json` и `package-lock.json`.
2.  Запустит скрипт для обновления версии в `README.md` и `README.ru.md`.
3.  Создаст git-коммит и тег.
...
*   **Сообщить об ошибке / Предложить функцию:** [GitHub Issues](https://github.com/SfilD/flux-node-manager/issues)

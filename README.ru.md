...
>
> В настоящее время веб-интерфейс нод на базе **ArcaneOS** был кардинально переработан и пока не полностью функционален.
> Текущая версия приложения сохраняет свою работоспособность и функционал **исключительно для Legacy-нод** (стандартный FluxOS).
> Поддержка ArcaneOS будет рассмотрена после стабилизации её веб-интерфейса.

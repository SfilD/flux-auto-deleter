...
Приложение хранит данные сессий (авторизацию на нодах) в отдельной системной папке. Если вы хотите полностью удалить все следы использования или сбросить все авторизации, запустите входящий в комплект скрипт `clean-session.bat` или вручную удалите следующую папку:

> **Примечание:** Скрипт работает как в CMD, так и в PowerShell. Права администратора НЕ требуются, но вы **должны закрыть приложение** перед запуском скрипта, чтобы избежать ошибок доступа.
